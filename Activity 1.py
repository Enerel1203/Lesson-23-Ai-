...
TH = 0.72
DEMOS = [('how do i delete my account', 'how do i remove my account'),
         ('start the game', 'begin the game'),
         ('nearest hospital to me', 'closest clinic to me'),
         ('mobile games are getting bigger by size', 'mobile games are expanding in size'),
...
def smart_score(base, q1, q2, strong):
    ...
    negA= ['no', 'not', 'never', 'without', 'cant', "can't", 'cannot', 'dont', "don't", 'do not', 'wont', 'will not', "won't", "n't"]
    ...
    neg_pen= 0.12 if has_any(q1, negA) and has_any(q2, negA) else 0
    opp_pen= 0.12 if ((has_any(q1, [a]) and has_any(q2, [b])) or (has_any(q1, [b]) and has_any(q2, [a])) for a, b in oppA) else 0
...
def show_results(s):
    print(f"Result of Similarity: {round(s*100, 1)}% [{bar(s)}] → {label(s)}")
...
def show_flow(q1, q2):
    ...
    conn= {'a', 'an', 'to', 'the', 'of', 'in', 'on', 'is', 'am', 'are', 'do', 'did', 'does', 'my', 'me', 'it'}

    least=sorted({w for w in raw if len(w)<=2 not in conn})
...
def main():
    print('Type Question 1 to Question 2. Then you\'ll see 2 RANDOM demo pairs.')
    ...
    while True:
        ...
        try:
            ...
            for i, (d1, d2) in enumerate(random.sample(DEMOS, 2), 1): run (d1, d2, f"Demo pair {i}: ")
